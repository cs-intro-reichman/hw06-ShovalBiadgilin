...
public class Runigram {
	...
	public static Color[][] flippedHorizontally(Color[][] image) {
		...
		Color [][] flippedHori=new Color[rows][cols];
		for(int i=0; i<rows; i++){ 
			for(int j=0; j<cols; j++){ 
				flippedHori[i][j]=image[i][cols-1-j];
			}
		}
		return flippedHori;
	}
	...
	public static Color[][] flippedVertically(Color[][] image){
		...
		for(int i=0; i<rows; i++){ 
			for(int j=0; j<cols; j++){ 
				flippedVer[i][j]=image[rows-1-i][j];
			}
		}
		return flippedVer;
	}
	...
	public static Color[][] grayScaled(Color[][] image) {
		...
		Color[][] greyImage=new Color [row][col];
		for(int i=0; i<row;i++){
			for (int j=0; j<col; j++){
	...
	public static Color[][] scaled(Color[][] image, int width, int height) {
		int row = image.length;
		int col=image[0].length;
		Color[][] scaling=new Color[height][width];
		for(int i=0;i<height;i++){ 
			for (int j=0; j<width;j++){
				int newWidth= i* row/height;
				int newHeight= j* col/width;
				scaling[i][j]= image [newWidth][newHeight];
			} 
		}
		return scaling;
	}
	...
	public static Color[][] blend(Color[][] image1, Color[][] image2, double alpha) {
		...
		Color[][] imageMorph=new Color [row][cols];
		for(int i=0; i<row;i++){ 
			for(int j=0; j<cols;j++){ 
				imageMorph[i][j]=blend(image1[i][j], image2[i][j],alpha);
			}
		}
		return imageMorph;
	}
	...
	public static void morph(Color[][] source, Color[][] target, int n) {
		...
		if(row1!=row2 || cols1!=cols2){
			target = scaled(target, row1, cols1);
		}
		int i=0;
